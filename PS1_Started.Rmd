...
```
### Group Member 1: Charles
### Group Member 2: Kayko
### Group Member 3: Josh
...
**Answer:**
```{r reg 1}
first_reg <- lm(out$Y ~ out$V1 + out$V2, out)
summary(first_reg)


# Our estimates of Beta_1 and Beta_2 are not biased.
# The population the population value of Beta_1 (3) is  within our 95% confidence interval for Beta_1(2.871, 3.076), and the value of Beta_2 (6) is within our 95% confidence interval for Beta_2(5.912, 6.117).

# There is a slight bias in the two variables, but the bias is negligible for both.
# we can't reject the null hypothesis that the mean is 
...
```{r reg 2}
second_reg <- lm(Y ~ V1, out)
summary(second_reg)


# Yes, Beta_1 is biased because the difference between the estimator and the population value is not 0. The population value for Beta_1(3) is not within our 95% confidence interval for Beta_1(7.548, 8.021).

# Yes, Beta_1 is biased because the difference between the estimator and the population value is not 0
...
**Answer:**
```{r reg 3}
intermediate_value <- coef(first_reg)[3]*out$V2
adjY <- out$Y - intermediate_value
third_reg <- lm(adjY ~ out$V1)
summary(third_reg)

#This variable is not biased, as the population coefficient Beta_1(3) is within the 95% confidence interval of the estimated coefficient Beta_1(2.926, 3.022).

#This variable is not biased, the population coefficient is within the 95% confidence interval of the estimated coefficient
#the adjY variable makes the estimation of V1 more accurate.
...
plotted1<-ggplot(out, aes(V1, y = value, color = variable)) + 
    geom_point(aes(y = Y, col = "Y"), color = "blue") + 
    geom_smooth(method='lm', aes(y = Y, col = "Y"), color = "blue") + 
    ggtitle("Y found for each V1") + ylab("Y")
    
plotted2<-ggplot(out, aes(V1, y = value, color = variable)) +
    geom_point(aes(y = adjY, col = "adjY"), color = "red")+
    geom_smooth(method='lm', aes(y = adjY, col = "Y"), color = "red") + 
    ggtitle("Adj Y found for each V1") + ylab("Adj Y")
...
#    geom_point(aes(y = adjY, col = "adjY", shape = "adjY"))+
#    geom_smooth(method='lm', aes(y = Y, col = "Y"))+
#    geom_smooth(method='lm', aes(y = adjY, col = "adjY"))

# plotted1<-ggplot(out, aes(V1, y = value, color = variable)) + 
#     geom_point(aes(y = Y, col = "Y")) + 
#     geom_smooth(method='lm', aes(y = Y, col = "Y_regLine"))
# 
# plotted2<-ggplot(out, aes(V1, y = value, color = variable)) + 
#     geom_point(aes(y = adjY, col = "adjY"))+
#     geom_smooth(method='lm', aes(y = adjY, col = "adjY_regLine"))
# 
# plotted3<-ggplot(out, aes(V1, y = value, color = variable)) + 
#     geom_smooth(method='lm', aes(y = Y, col = "Y"))+
#     geom_smooth(method='lm', aes(y = adjY, col = "adjY'"))
#     
# 
...
#Kayko's Code:
library(ggpubr)
p1 <- ggplot(out, aes(V1, y = value)) + 
    geom_point(aes(y = Y, col = "Y"), alpha=0.4) + 
    geom_point(aes(y = adjY, col = "adjY"), alpha=0.4)+ 
    geom_smooth(method='lm', aes(y = Y, col = "Y"))+
    geom_smooth(method='lm', aes(y = adjY, col = "adjY")) +
	scale_color_manual(values=c("#FFB81C", "#003594")) +
...
p2 <- ggplot(out, aes(x=Y)) + 
	geom_density(color="#00205B", fill="#003594", alpha=0.6) + 
	theme(axis.text.x = element_blank(),  axis.text.y = element_blank()) + 
	scale_x_continuous(limits=c(-35, 35)) + 
	scale_y_continuous(limits=c(0, 0.125)) +
	coord_flip() +
	theme_void()
p3 <- ggplot(out, aes(x=adjY)) + 
	geom_density(color="#B58500", fill="#FFB81C", alpha=0.6) + 
	theme(axis.text.x = element_blank(),  axis.text.y = element_blank()) + 
	scale_x_continuous(limits=c(-35, 35)) + 
	coord_flip() +
	theme_void()
improved <- ggarrange(p1, p2, p3, widths=c(4, 0.5, 0.5), ncol=3)
annotate_figure(improved, top = text_grob("Omitted Variable Bias in Y vs. adjY", face = "bold", size = "14"))
